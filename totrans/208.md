...
现在让我们将一个注意力层添加到之前创建的 RNN 网络中。`create_RNN_with_attention()`函数现在在网络中指定了一个 RNN 层、一个注意力层和一个稠密层。确保在指定SimpleRNN时设置`return_sequences=True`，这将返回所有先前时间步的隐藏单元输出。
