...
时间序列中没有输入和输出功能的概念。相反，我们必须选择要预测的变量，并使用特征工程来构建将用于对未来时间步骤做出预测的所有输入。
...
输入和输出数据之间的关系存在复杂性。在时间序列的情况下，没有输入和输出变量的概念;我们也必须发明这些并从零开始构建有监督的学习问题。
...
我们可以使用窗口做更多事情，而不是包含原始值。在下一节中，我们将介绍包含在窗口中汇总统计量的功能。

## 滚动窗口统计

添加原始滞后值之外的步骤是添加前一时间步的值的摘要。

我们可以计算滑动窗口中值的汇总统计量，并将这些统计量包含在数据集中。也许最有用的是前几个值的平均值，也称为滚动均值。
...
我们可以计算更多的统计量，甚至可以用不同的数学方法计算“窗口”的定义。

下面是另一个示例，显示窗口宽度为 3，数据集包含更多摘要统计量，特别是窗口中的最小值，平均值和最大值。

您可以在代码中看到我们明确指定滑动窗口宽度作为命名变量。这使我们可以在计算系列的正确位移和指定`rolling()`函数的窗口宽度时使用它。

在这种情况下，窗口宽度为 3 表示我们必须将系列向前移动 2 个时间步长。这使得前两行为 NaN。接下来，我们需要计算每个窗口有 3 个值的窗口统计量。在我们甚至从窗口中的系列中获得足够的数据以开始计算统计量之前，它需要 3 行。前 5 个窗口中的值如下：
