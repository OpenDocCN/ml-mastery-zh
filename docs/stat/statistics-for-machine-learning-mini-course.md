...
`pearsonr()`NumPy 函数可用于计算两个变量样本的 Pearson 相关系数。
...
Student's t-test 可以通过`ttest_ind()`SciPy 函数在 Python 中实现。
...
这可以使用`confint()`Statsmodels 函数在 Python 中实现。
...
该测试可以通过`mannwhitneyu()`SciPy 函数在 Python 中实现。
