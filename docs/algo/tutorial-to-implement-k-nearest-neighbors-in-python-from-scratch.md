...
它是一个标准数据集，其中物种对于所有实例都是已知的。因此，我们可以将数据拆分为训练和测试数据集，并使用结果来评估我们的算法实现。该问题的良好分类准确度高于 90％正确，通常为 96％或更高。
...
您可能希望探索此算法的更多扩展。另外两个想法包括支持对预测的 k-最相似实例的距离加权贡献以及用于搜索类似实例的更高级的基于数据树的结构。
